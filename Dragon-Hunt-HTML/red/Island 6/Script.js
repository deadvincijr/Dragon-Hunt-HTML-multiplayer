...
document.addEventListener('keydown', function(event) {
  ...
  // --- ADDED RELIC 4 and 5 ---
  const relic4 = document.getElementById('relic4');
  const relic4Rect = relic4 ? relic4.getBoundingClientRect() : null;

  const relic5 = document.getElementById('relic5');
  const relic5Rect = relic5 ? relic5.getBoundingClientRect() : null;

  // --- Define the collision handler function ---
  // This function checks against all relics and performs the same action.
  const handleRelicCollision = (futureRect) => {
      ...
      // --- ADDED RELIC 4 and 5 CHECKS ---
      if (relic4Rect && areRectsOverlapping(futureRect, relic4Rect)) {
          alert("You found the single-use dragon staff");
          console.log("Touching Relic 4!");
          // Put relic 4 found in the cloud vars for this person
      }

      if (relic5Rect && areRectsOverlapping(futureRect, relic5Rect)) {
          alert("You found a weather staff");
          console.log("Touching Relic 5!");
          // Put relic 5 found in the cloud vars for this person
      }
  };
